...
app.post('/api/home-tarif', upload.single('image'), (req, res) => {
	...
	fs.readFile(dataPath, 'utf8', (err, data) => {
		...
		const jsonData = JSON.parse(data);
		const newId = jsonData['home-tarif'].length > 0 
...
app.delete('/api/home-tarif/:id', (req, res) => {
	...
	fs.readFile(dataPath, 'utf8', (err, data) => {
		...
		const jsonData = JSON.parse(data);
		const tarifIndex = jsonData['home-tarif'].findIndex(t => t.id === tarifId);
		...
		fs.writeFile(dataPath, JSON.stringify(jsonData, null, 2), (err) => {
			...
			fs.unlink(imagePath, (err) => {
				if (err) {
					...
					return res.status(200).json({ message: 'Тариф удален, но изображение не удалось удалить', tarif: tarifToDelete });
				}
				res.status(200).json({ message: 'Тариф и изображение успешно удалены', tarif: tarifToDelete });
			});
		});
	});
});
...
app.put('/api/home-tarif/:id', upload.single('image'), (req, res) => {
	...
	fs.readFile(dataPath, 'utf8', (err, data) => {
		...
		existingTarif.accessSpeed = req.body.accessSpeed || existingTarif.accessSpeed;
		...
		fs.writeFile(dataPath, JSON.stringify(jsonData, null, 2), (err) => {
			...
			res.status(200).json({ message: 'Тариф успешно обновлен', tarif: existingTarif });
		});
	});
});
...
app.post('/api/news', upload.single('image'), (req, res) => {
	...
	fs.readFile(dataPath, 'utf8', (err, data) => {
		...
		const jsonData = JSON.parse(data);
		const newId = jsonData['news'].length > 0
...
app.put('/api/news/:id', upload.single('image'), (req, res) => {
	...
	fs.readFile(dataPath, 'utf8', (err, data) => {
		...
		fs.writeFile(dataPath, JSON.stringify(jsonData, null, 2), (err) => {
			...
			res.status(200).json({ message: 'Новость успешно обновлена', news: existingNews });
		});
	});
});
	...
	fs.readFile(dataPath, 'utf8', (err, data) => {
		...
		fs.writeFile(dataPath, JSON.stringify(jsonData, null, 2), (err) => {
			...
			fs.unlink(imagePath, (err) => {
				if (err) {
					console.error('Ошибка удаления изображения:', err);
					return res.status(200).json({ message: 'Новость удалена, но изображение не удалось удалить', news: newsToDelete });
				}
				res.status(200).json({ message: 'Новость и изображение успешно удалены', news: newsToDelete });
			});
		});
	});
});
...
app.post('/api/faq', (req, res) => {
	...
	fs.readFile(dataPath, 'utf8', (err, data) => {
		...
		const newId = jsonData['faq'].length > 0
...
app.put('/api/faq/:id', (req, res) => {
	...
	fs.readFile(dataPath, 'utf8', (err, data) => {
		...
		fs.writeFile(dataPath, JSON.stringify(jsonData, null, 2), (err) => {
			...
			res.status(200).json({ message: 'FAQ успешно обновлена', faq: existingFaq });
		});
	});
});
...
app.delete('/api/faq/:id', (req, res) => {
	...
	fs.readFile(dataPath, 'utf8', (err, data) => {
		...
		fs.writeFile(dataPath, JSON.stringify(jsonData, null, 2), (err) => {
			...
			res.status(200).json({ message: 'FAQ успешно удалена', faq: faqToDelete });
		});
	});
});
